...
while True:
    ...
    if status:
        ...
        np.where(camera, 640, 480)
        np.where(mountain, 640, 480)
        # invertendo a máscara
        
        # bitwise_and - operação para extrair o primeiro plano / pessoa
        mountain = cv2.bitwise_not(camera)
        cv2.imshow("camera_1AT", camera)
        cv2.imshow("mountain_2", mountain)
        # imagem final
        final_image = np.where(camera == 0, camera, mountain)
